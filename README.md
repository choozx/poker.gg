...
이 파일 하나에 모든 핸드(원본 포함)·AI 분석·**뱅크롤(토너 결과·바이인/상금·강제링크)**이 다 들어 있다.
원본 txt 파일은 옮길 필요 없다 — DB에 raw가 저장되어 있어 `--rebuild`도 가능하다.

> **다른 컴퓨터에서 그대로 보기** = `git clone`/`pull`(코드) + `hands_db.json` 복사. 끝.
...
| `hands_db.json` | (자동 생성) 핸드·AI분석·뱅크롤 저장소 — 백업 권장 |
