...
CoinPoker 토너먼트 플레이를 위한 로컬 분석 도구 — 핸드 히스토리(txt)를 읽기 쉽게 변환하고,
핸드별 AI(Claude) 분석 · 통계 대시보드 · **뱅크롤(실제 손익) 관리**까지 한곳에서.
...
- **🎯 문제 풀기** — 내가 자주 실수하는 스팟에서 문제를 출제하는 연습 모드.
  **실제 내 핸드**를 히어로의 결정 직전에서 잘라 보여주고(그 뒤 액션·다음 스트리트·쇼다운·결과는
  전부 가려짐) 액션을 고르면 **AI가 채점**([좋음/무난/의문/실수] + 근거 + 최선 + 기억할 것).
  채점이 끝난 뒤에야 "실제로는 이렇게 쳤고 결과는 이랬다"가 공개됨.
  - **약점 스팟 선정 2축** — ① *휴리스틱*: 큰 손실/쇼다운 패배/올인 패배를 포지션×스택대로 묶음.
    ② *통계 이탈*: 포지션×스택대 **RFI·디펜스 빈도**가 기준선에서 벗어난 구간
    (`--rebuild` 필요 — 안 하면 ①만 동작하고 안내가 뜸)
  - **출제 범위 필터** — 3줄 토글, 모두 복수 선택:
    **포지션**(UTG/MP/CO/BTN/SB/BB · `MP`는 MP1~MP3 포함 · 선택한 게 **히어로 포지션**인 스팟만),
    **스택**(<15 / 15–25 / 25–40 / 40bb+),
    **스트릿**(프리플랍/플랍/턴/리버 — 히어로가 **그 스트릿에서 결정하는** 지점으로 출제).
    프리플랍을 빼면 통계 이탈(📊) 스팟은 프리플랍 지표라 자동 제외됨
  - **출제는 토큰 0** (로컬에서 자름). AI는 채점 1회만. 같은 문제·같은 선택은 캐시 재사용으로 0회
  - 한 스팟의 실제 핸드를 다 풀면 **AI가 같은 성격의 연습 문제를 새로 생성**해 이어서 출제
  - 성적표: 정답률·등급 분포·스트리트별 정답률
...
- **연습 트레이너 고도화** (기본형은 🎯 문제 풀기로 구현 완료) — 남은 것:
  - **Nash/ICM 차트 판정** — 숏스택 프리플랍 푸시/폴드는 AI 대신 차트로 정확 판정(현재는 전부 AI 채점).
  - **간격 반복** — 틀린 스팟을 주기적으로 다시 출제(현재 출제 가중치는 리크 크기만 반영).
  - **AI 등급 축 추가** — 분석된 핸드의 `[의문]/[실수]`를 3번째 스팟 선정 축으로
    (분석 표본이 충분히 쌓이면. 현재는 휴리스틱 + 통계 이탈 2축).
- **리크 자동 진단 UI** — 포지션×스택별 이탈 탐지 자체는 🎯 문제 풀기의 *통계 이탈* 축으로
  들어가 있음. 이걸 통계 탭에 독립 리포트로 노출하는 건 미구현.
...
| `quiz.py` | 🎯 문제 풀기 — 약점 스팟 탐지(휴리스틱·통계 이탈)·문제 출제·채점 캐시/성적 |
