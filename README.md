...
- **💰 뱅크롤** — **실제 돈($)** 손익 관리 (칩 EV와 별개 도메인 — 칩≠상금이라 분리).
  ...
  **실제 잔고 추적** — 잔고를 한 번 입력하면 이후 토너 손익을 자동 가산(스냅샷 방식, 리워드 차이는 재입력으로 보정).
  **바이인 추천** — ROI·ITM·추세 + **실제 잔고 기준 자금 가드**(분산 대비 적정 바이인)로 올려/유지/내려 제시.
...
- **바이인 추천 고도화** — 변동성 기반 **리스크 오브 루인 / 적정 뱅크롤**(몇 바이인 필요한지) 계산.
...
| `bankroll.py` | 뱅크롤(실제 돈) — 시트 이주·핸드 매칭·캠페인 트리·win/loss 판정·바이인 추천·잔고 추적 |
